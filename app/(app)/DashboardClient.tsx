  ...
  return (
    <div>
      <header className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-plum-darkest">Calendars</h1>
          ...
          {!persistent && (
            <p className="mt-2 max-w-2xl rounded-md border border-orange-light bg-orange-lightest px-3 py-2 text-xs text-orange-darker">
              No persistent storage connected — bookings live in memory and reset on every
              redeploy or cold start. Connect a Redis database (Storage tab in your Vercel
        ...
        <div className="flex shrink-0 gap-2">
          <button
            ...
            disabled={busy}
            className="rounded-md border border-plum-lighter/70 bg-white px-3 py-2 text-sm font-medium text-plum-dark hover:bg-plum-lightest disabled:opacity-50"
          >
            Load demo calendars
          </button>
          <button
      ...
      {calendars.length === 0 ? (
        <div className="rounded-lg border border-dashed border-plum-lighter bg-white p-10 text-center text-violet-darker">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {calendars.map((cal) => (
            <div
              key={cal.id}
              className="flex items-start justify-between gap-3 rounded-lg border border-plum-lighter/40 bg-white p-4 shadow-sm transition hover:shadow-md"
            >
              <Link href={`/calendars/${cal.id}`} className="flex-1">
                <div className="flex items-center gap-2 text-lg font-bold text-plum-darkest">
                  <span>{CALENDAR_TYPE_EMOJI[cal.type]}</span>
                  <span>{cal.name}</span>
                </div>
                <div className="mt-1 text-xs uppercase tracking-wide text-violet">
                  {CALENDAR_TYPE_LABELS[cal.type]}
                </div>
                <div className="mt-2 text-xs text-violet-darker">
                  {cal.slotMinutes}-min slots &middot; {cal.timezone}
                </div>
              </Link>
              <button
                onClick={() => deleteCalendar(cal.id)}
                className="text-xs text-violet hover:text-orange-darker"
  ...
  return (
    <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        <h2 className="text-lg font-bold text-plum-darkest">Add a calendar</h2>
        <form onSubmit={submit} className="mt-4 space-y-4">
          <div>
            <label className="block text-sm font-medium text-plum-darkest">Business name</label>
            <input
              autoFocus
              value={name}
          ...
          <div>
            <label className="block text-sm font-medium text-plum-darkest">Type</label>
            <select
              value={type}
          ...
          <p className="text-xs text-violet-darker">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-md border border-plum-lighter/70 px-3 py-2 text-sm text-plum-dark"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className="rounded-md bg-orange px-3 py-2 text-sm font-semibold text-white hover:bg-orange-dark disabled:opacity-50"
            >
